...
const S = `
  ...
  .s-en_revision{color:#D97706;background:rgba(217,119,6,.08);border:1px solid rgba(217,119,6,.2)}
  .s-revertida  {color:#7C3AED;background:rgba(124,58,237,.08);border:1px solid rgba(124,58,237,.2)}
...
const statusMap = {
  ...
  en_revision: { label: 'En Revisión', cls: 's-en_revision', Icon: Clock },
  revertida:   { label: 'Revertida',   cls: 's-revertida',   Icon: RefreshCw },
};
  ...
  return (
    <div className="tx-row">
      <div style={{ flex: 1, minWidth: 0, marginRight: '1rem' }}>
        <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '.875rem', fontWeight: 600, color: '#0D1F3C', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {tx.description}
        </p>
...
const AdminTransactions = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('transactions_with_user_details')
      .select('*')
  ...
  // ✅ Autorizar: cambia status a 'completed' directamente, sin Edge Function
  const handleAuthorize = async (tx) => {
    ...
    try {
      ...
      toast({ title: "Éxito", description: "Transacción autorizada." });
      loadData();
    } catch(e) {
  ...
  // ✅ Rechazar: usa RPC con solo p_transaction_id, devuelve saldo automáticamente
  const handleReject = async (tx) => {
    ...
    try {
      ...
      const { error: balanceError } = await supabase
        .from('profiles')
        .update({ balance: supabase.rpc('get_balance', { uid: tx.user_id }) })
        .eq('id', tx.user_id);

      // Incrementar balance directamente con rpc de sql
      const { error: refundError } = await supabase.rpc('refund_transaction', {
